    ...
    return (
      <div className="bg-white rounded-lg shadow-md p-6">
        ...
        <div className="text-center text-gray-500 py-8">
          ...
          <p className="text-sm mt-2">{t('analytics_hint')}</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        <div className="bg-blue-50 p-4 rounded-lg">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="font-medium text-blue-800">{t('efficiency')}</span>
          </div>
          <div className="text-2xl font-bold text-blue-900">
            {efficiencyMetrics.routeEfficiency.toFixed(1)}%
          </div>
          <div className="text-sm text-blue-700">{t('optimization_gain')}</div>
        </div>

        <div className="bg-green-50 p-4 rounded-lg">
          <div className="flex items-center gap-2 mb-2">
            <Fuel className="text-green-600" size={20} />
            <span className="font-medium text-green-800">{t('fuel_cost')}</span>
          </div>
          <div className="text-2xl font-bold text-green-900">
            ${environmentalData.cost.toFixed(2)}
          </div>
          <div className="text-sm text-green-700">{t('fuel_cost')}</div>
        </div>

        <div className="bg-purple-50 p-4 rounded-lg">
          <div className="flex items-center gap-2 mb-2">
            <Clock className="text-purple-600" size={20} />
            <span className="font-medium text-purple-800">{t('avg_time_per_stop')}</span>
          </div>
          <div className="text-2xl font-bold text-purple-900">
            {efficiencyMetrics.avgTimePerStop.toFixed(1)}m
          </div>
          <div className="text-sm text-purple-700">{t('avg_time_per_stop')}</div>
        </div>

        <div className="bg-red-50 p-4 rounded-lg">
          <div className="flex items-center gap-2 mb-2">
            <Leaf className="text-red-600" size={20} />
            <span className="font-medium text-red-800">{t('co2_emissions')}</span>
          </div>
          <div className="text-2xl font-bold text-red-900">
            {environmentalData.co2.toFixed(1)} kg
          </div>
          <div className="text-sm text-red-700">{t('co2_emissions')}</div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div>
          <h4 className="font-medium mb-4">{t('distance_per_segment')}</h4>
        ...
        {/* Speed Analysis */}
        <div>
          <h4 className="font-medium mb-4">{t('speed_per_segment')}</h4>
          <ResponsiveContainer width="100%" height={250}>
      ...
      <div className="mt-6 p-4 bg-green-50 rounded-lg">
        <h4 className="font-medium text-green-800 mb-3 flex items-center gap-2">
          <Leaf size={20} />
          {t('environmental_summary')}
        </h4>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
          <div>
            <span className="text-green-700 font-medium">{t('fuel_cost')}:</span>
            <div className="text-green-900">{environmentalData.fuel.toFixed(2)} liters</div>
          </div>
          <div>
            <span className="text-green-700 font-medium">{t('co2_emissions')}:</span>
      ...
      {route.optimization_stats && (
        <div className="mt-6 p-4 bg-blue-50 rounded-lg">
          <h4 className="font-medium text-blue-800 mb-3">{t('optimization_details')}</h4>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
            <div>
              <span className="text-blue-700 font-medium">{t('algorithm')}:</span>
              <div className="text-blue-900">{route.optimization_stats.algorithm_used}</div>
            </div>
            <div>
              <span className="text-blue-700 font-medium">{t('computation_time')}:</span>
              <div className="text-blue-900">{route.optimization_stats.computation_time.toFixed(2)}s</div>
            </div>
            <div>
              <span className="text-blue-700 font-medium">{t('iterations')}:</span>
